...
#    
        ...
        # background
    G['background']=visual.Rect(win, width=2, height=2,fillColor=[-0.1,-0.1,-0.1],lineWidth=0)
   
    G['crosshair'] = visual.TextStim(win, text='+', units='norm', pos=(0, 0))
    
    items=[G['background']] #, G['crosshair'] ]
    
    SCALING=G['v']['EX_SCALING']
...
def wait_for_key(G, message):
    event.clearEvents()
    win=G['win']
    eh=G['eh']
    
    testinstr=visual.TextStim(win, message, pos=(0, 0), units='norm')
...
def instr_screen(G):
    ...
    eh=G['eh']
    # BUTTONS=G['v']['BUTTONS']
    contstim=visual.TextStim(win, 'Eyes Open...',pos=(0.0, 0.90), height=0.08, units='norm')
    startstim=visual.TextStim(win, '(Press to continue)',pos=(0.0, 0.0), height=0.12, units='norm')
...
def eo_stim(G):
    ...
    # G['eh'].send_message('beo')
    # EYESCLOSED_TIME=G['v']['EYESCLOSED_TIME']
    EYESOPEN_TIME = G['v']['EYESOPEN_TIME']
    win=G['win']
    eh=G['eh']
    incor_clock=clock.Clock()
    contstim=visual.TextStim(win, 'Press to continue...',pos=(0.0, 0.90), height=0.08, units='norm')
    ...
    incor_clock=clock.Clock()
    while incor_clock.getTime()<float(EYESOPEN_TIME):
        G['background'].draw()
        G['crosshair'].draw()
        win.flip()
